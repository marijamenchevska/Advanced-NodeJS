...
export class ArtistsController {
    ...
    @ApiForbiddenResponse({ description: 'You are unauthorized to access this page' }) 
    @ApiInternalServerErrorResponse({ description: `The artist with the requested ID doesn't exist or invalid ID` })
    @Put('/:id')
    @Roles(Role.MODERATOR)
    updateArtist(@Param('id') id: string, @Body() body: ArtistUpdateDto, @CurrentUser() user: ICurrentUser): Promise<Artist> {
    ...
    @HttpCode(204)
    @Delete('/:id')
    deleteArtist(@Param('id') id: string, @CurrentUser() user: ICurrentUser): Promise<void> {
